...
import {
  loadGuestSolvedProblemIds,
  recordGuestSolvedProblem,
} from "@/lib/utils/guestProgress";
import {
  hapticError,
  hapticGoldIncrease,
  hapticSuccess,
...
export function useProblemSolver(problem: Problem | null) {
  ...
  const [status, setStatus] = useState<
    "idle" | "started" | "submitted" | "success" | "fail"
  >("idle");
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const isCorrect =
      answer.toUpperCase().trim() ===
        (problem.correctAnswer ?? "").toUpperCase().trim() &&
      answer.trim() !== "";
    ...
    if (isCorrect) {
      ...
      switch (currentStreak) {
        case 2:
          bonusGold = Math.round(problem.rewardGold + 10);
          break;
        case 3:
          bonusGold = Math.round(problem.rewardGold + 20);
          bonusEnergy = 1;
          break;
        case 4:
          bonusGold = Math.round(problem.rewardGold * 2 + 30);
          bonusEnergy = 1;
          break;
        case 5:
          bonusGold = Math.round(problem.rewardGold * 3 + 40);
          bonusEnergy = 2;
          break;
        case 6:
          bonusGold = Math.round(problem.rewardGold * 3.5 + 50);
          bonusEnergy = 2;
          break;
        case 7:
          bonusGold = Math.round(problem.rewardGold * 4 + 60);
          bonusEnergy = 3;
          break;
        case 8:
          bonusGold = Math.round(problem.rewardGold * 4.5 + 70);
          bonusEnergy = 3;
          break;
        case 9:
          bonusGold = Math.round(problem.rewardGold * 5 + 80);
          bonusEnergy = 4;
          break;
        case 10:
          bonusGold = Math.round(problem.rewardGold * 7 + 100);
          bonusEnergy = 5;
          break;
        default:
          if (currentStreak > 10) {
      ...
      if (isGuest) {
        recordGuestSolvedProblem(problem.id);
      }
  ...
  const handleNextProblem = useCallback(async () => {
    ...
    if (currentLevelProblems) {
      const unsolvedInCurrentLevel = currentLevelProblems.filter(
      ...
      const allCurrentLevelSolved = currentLevelProblems.every((p) =>
        solvedIds.has(p.id)
      );
    ...
    router.push("/learn");
  }, [problem, user, profile, router, addXp, isGuest]);
