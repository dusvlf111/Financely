...
export default function MyPage() {
  const { profile, updateProfile, logout, user, isGuest } = useAuth();
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      await Promise.allSettled([fetchProblems(), fetchSolvedProblems()]);
      if (!isCancelled) {
  ...
  async function handleDeleteAccount() {
    if (
      window.confirm(
        "정말로 계정을 삭제하시겠습니까? 모든 데이터는 영구적으로 삭제되며 복구할 수 없습니다."
      )
    ) {
    ...
    <div className="max-w-[768px] mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-neutral-400">
              {totalCount > 0 ? totalCount - solvedCount : 0}
            </div>
      ...
      <section className="card-animated animate__animated animate__fadeInUp stagger-2 p-6 mb-6">
        ...
        <div className="flex gap-2 mb-4">
          ...
          >
            안 푼 문제 ({totalCount > 0 ? totalCount - solvedCount : 0})
          </button>
        </div>

        {/* 문제 리스트 */}
        {isProblemsLoading ? (
          <div className="py-12 flex flex-col items-center text-neutral-500 text-sm">
            <svg
            ...
            <span className="mt-3">문제를 불러오는 중입니다...</span>
          </div>
          ...
          <>
            ...
            {filteredProblems.length > 3 && (
              ...
              >
                {isProblemsExpanded
                  ? "접기 ▲"
                  : `더보기 (${filteredProblems.length - 3}개) ▼`}
              </button>
            )}
          </>
        )}
